...
const UserDocumentsPage: React.FC = () => {
    ...
    const handleAdd = (docType: 'Договор' | 'Протокол') => {
        addUserDocument(decodedEmail, docType);
        setDocs(getUserDocuments(decodedEmail)); // Re-fetch to update state
    };
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                        ...
                        <div className="flex gap-2 flex-shrink-0">
                             <button onClick={() => handleAdd('Договор')} className="bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-blue-700 transition-colors flex items-center gap-2">
                                <PlusIcon className="w-4 h-4" /> Договор
                            </button>
                             <button onClick={() => handleAdd('Протокол')} className="bg-green-600 text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-green-700 transition-colors flex items-center gap-2">
                    ...
                    {docs.length > 0 ? (
                        ...
                        <div className="text-center py-10 bg-gray-50 rounded-lg">
                            ...
                            <p className="mt-1 text-sm text-gray-500">Вы можете добавить договор или протокол с помощью кнопок выше.</p>
                        </div>
                    )}
                </div>
            </div>
